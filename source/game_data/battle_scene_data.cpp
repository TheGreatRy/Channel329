...
{
    enemy = new Sprite("sprite/cam_battle_neut", 0, 0, 128, 128, 0);       
    attacker = new Sprite("sprite/john_tutorial", 1, 1, 128, 128, 0);       
    test_battle = new Battle();

    enemy->LoadSprite(0, 0, 0, {new Position(10.0f, 10.0f)}, false, 0);
    attacker->LoadSprite(1,1,1, {new Position(40.0f, 10.0f)}, false, 0);
    ...
    // battle_top_bg = new Background(2, BgType_Bmp8, BgSize_B8_128x128, 3, 0, true);
    // battle_bottom_bg = new Background(2, BgType_Bmp8, BgSize_B8_256x256, 4, 0, false);

    npc = new Character(npc_temp, "JOHN NPC", CHARACTER_TYPE_NPC);
    cam_attk = new Character(attack_temp, "CAMERON", CHARACTER_TYPE_NPC);
    
    // battle_top_bg->LoadBackground( 0, battle_topBitmap, battle_topBitmapLen, battle_topPal, battle_topPalLen); 
