...
Map::Map(Tileset* tileset_info, int map_width, int map_height, const int16_t tile_id_arr[])
{
    ...
    for (int current_x = 0; current_x < (m_map_width * m_map_height); current_x++) //current x == 0 - 30*20
    {
        ...
        if (current_x < m_map_width) //x < 20
            m_coordinates.push_back(new Coordinate<int16_t>(current_x, current_y, tile_id_arr[current_x]));
            //x = 0 -19, y = 0;

        // if we are past the first row
        else if (current_x >= m_map_width) //x >= 20
        {
            int row_reset = current_x % m_map_width; // x % 20 (should cycle every 20 values)
            if (row_reset == 0)
...
void Map::DrawMap(int scroll_x, int scroll_y, bool& can_move_u, bool& can_move_d, bool& can_move_l, bool& can_move_r)
{
    ...
    int width_tile_amt = (SCREEN_WIDTH / m_tileset_info.m_sprite_w);

    int height_tile_amt = (SCREEN_HEIGHT / m_tileset_info.m_sprite_h);
    ...
    for (int current_x = 0; current_x < (m_map_width * m_map_height); current_x++) // until 17*13
    {
        ...
        //draws entire map
        draw_x_pos = scroll_x + m_coordinates.at(current_x)->m_x * m_tileset_info.m_sprite_w;
        draw_y_pos = scroll_y + m_coordinates.at(current_x)->m_y * m_tileset_info.m_sprite_h;
