...
void Scene::DeleteAllTextures()
{
    ...
    {
        for (Animation *animation : actor->m_sprite_animations)
            glDeleteTextures(1, &animation->m_spritesheet.m_texture_id);
    }
}
...
void Scene::DrawScene(int scroll_x, int scroll_y, bool &can_move_up, bool &can_move_down, bool &can_move_left, bool &can_move_right)
{
    ...
    while (1)
    {
        ...
        if (m_main_consoles.size() > 0)
        {
            for (TextConsole *main_con : m_main_consoles)
            {
                // if (main_con->m_text_console_type == TEXT_CON_TYPE_MAIN_TALK)
                main_con->DisplayTextConsole(&main_con->m_print_console);
            }
        }
        if (m_sub_consoles.size() > 0)
        {
            int index = 0;  
